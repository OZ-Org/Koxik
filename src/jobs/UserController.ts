...
import type { Transaction, BackpackItem, BackpackType } from '@misc/types.js';
...
export class UserController {
  ...
  static async create(discordId: string) {
    if (await this.exists(discordId)) {
      return { success: false, alreadyExists: true };
    }

    const newUser = await this.get(discordId); // get() creates if not exists
   ...
   */
  static async update(discordId: string, data: Partial<typeof user.$inferInsert>) {
  ...
  static async addBalance(discordId: string, amount: number) {
    await this.get(discordId); // Ensure user exists
  ...
  static async removeBalance(discordId: string, amount: number) {
    const userData = await this.get(discordId);
   ...
   */
  static async updateBackpackItem(discordId: string, itemId: string, updateFn: (item: BackpackItem) => BackpackItem) {
    const userData = await this.get(discordId);
    const currentBackpack = (userData.backpack as BackpackType) || [];
